...
const AppProvider = ({ children }) => {
    ...
    const fetchDancehallData = async () => {
        try {
            const response = await axios(`${DISCOGS_URL}/database/search?&style=dancehall&format=vinyl&token=${DISCOGS_KEY}`)
            const data = response.data.results
            if (data) {
    ...
    const removeFromCart = (product) => {
        const updatedCart = state.cart.filter((currentProduct) => currentProduct.name !== product.name)
        dispatch({
